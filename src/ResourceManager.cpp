...
const sf::Music* ResourceManager::getOrLoadMusic(const std::string& filename)
{
    auto it = m_loadedMusics.find(filename);
    if (it != m_loadedMusics.end())
        return &it->second;

    auto res = m_loadedMusics.emplace(filename, sf::Music());
    if (!res.second)
        return nullptr;

    sf::Music* pMusic = &res.first->second;
    if (!pMusic->openFromFile(getAssetPath(filename)))
        return nullptr;
    return pMusic;
}
